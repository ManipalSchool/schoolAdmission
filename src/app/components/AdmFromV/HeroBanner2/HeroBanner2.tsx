    ...
    <div className="min-h-screen">
      ...
      <main className="relative min-h-screen flex  items-center justify-center">
        ...
        {/* <div className="absolute inset-0 z-0">
          ...
          >
            <SwiperSlide>
              <div className="relative h-[50vh] md:h-[93vh] w-full">
                <Image
                  src={websitebg1}
                  alt="Banner1"
                  fill
...
<div
 className="w-full min-h-screen px-1 sm:px-6 md:px-8 py-16 flex justify-center items-center bg-cover bg-center relative mt-[1rem] sm:mt-0 "  
 style={{
    backgroundImage: "url('/your-banner-image.jpg')", // 👈 Replace with your actual image path
  }}
>
  ...
  <motion.div
    ...
    viewport={{ once: true }}
    className="absolute top-[10rem] right-[0rem] sm:top-6 sm:right-6 md:top-[0rem] md:right-[2.5rem] lg2:top-[-3rem] lg2:right-[-2rem] w-20 h-20 md:w-24 md:h-24 lg2:w-36 lg2:h-36 bg-green-400 rounded-bl-full rounded-tl-full rounded-br-full opacity-100 z-10"
  />
  ...
  <motion.div
    ...
    viewport={{ once: true }}
    className="absolute bottom-[14.5rem] left-[-1.2rem] md:bottom-15 md:left-5 lg2:left-[65%] lg2:bottom-[10%] w-24 h-12 md:w-32 md:h-16 xl:w-36 xl:h-18 bg-[#FEA3CA] rotate-[220deg] rounded-t-full opacity-100 z-10"
  />
...
<motion.div
  ...
  className="bg-white  shadow-2xl rounded-2xl w-full max-w-[calc(100%-2rem)] sm:max-w-[600px] lg:max-w-[calc(100%-6rem)] px-8 sm:px-8 lg:px-12 py-12 md:py-20 md:mt-[-4rem] mt-[-5rem] relative z-20"
>
    {/* Headings */}
    <motion.h2
      initial={{ opacity: 0, y: -20 }}
      ...
      transition={{ duration: 0.5 }}
      className="text-2xl sm:text-3xl lg2:text-[1.8rem] xl:text-[3.5rem] leading-tight font-bold text-[#FB7824] text-center"
    ...
    <motion.h2
      ...
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.5 }}
      className="text-xl sm:text-2xl lg2:text-[1.5rem] xl:text-[2.5rem] leading-tight pb-4 font-bold text-gray-800 text-center"
      style={{ fontFamily: "Helvetica, Arial, sans-serif" }}
    >
      Now For 2025
    </motion.h2>

  <form onSubmit={handleSubmit(onSubmit)} className="w-full space-y-6 text-black">      {/* Student Name */}
      ...
      <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.4 }}>
        <label className="block text-sm text-gray-600">
          ...
          >
            <option value="" disabled className="!text-gray-400 text-base md:text-lg2 xl:text-xl">
      ...
      <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.1 }}>
        <input
          ...
          aria-label="Student Name"
          {...register("studentName", { required: "Student Name is required" })}
          className="w-full border-b border-black/[20%] focus:outline-none text-base sm:text-lg xl:text-xl"
        />
        {errors.studentName && <p className="text-red-500 text-xs sm:text-sm">{errors.studentName.message}</p>}
      </motion.div>
      ...
      <div className="justify-center flex">
        <button
          type="submit"
          className="w-full bg-[#FB7824] hover:bg-[#e7681e] cursor-pointer text-white py-2 px-4 rounded-md flex justify-center items-center transition-all"
          disabled={loading}
        >
